...
namespace plop::music {
	...
	static constexpr std::array<GmDrum, 47> k_gmDrums = { {
	...
	inline const char *gmDrumName( int note ) {
		for ( const auto &d : k_gmDrums )
			if ( d.note == note ) return d.name;
		return nullptr;
	}

	/// Returns the index in k_gmDrums for the given note, snapping to the nearest
	/// entry at or above the note if there is no exact match.
	inline int gmDrumIndexForNote( int note ) {
		for ( int i = 0; i < static_cast<int>( k_gmDrums.size() ); ++i )
			if ( k_gmDrums[ i ].note >= note ) return i;
		return static_cast<int>( k_gmDrums.size() ) - 1;
	}

	/// Returns the drum note number at the given index, clamped to valid range.
	inline int gmDrumNoteAtIndex( int idx ) {
		if ( idx < 0 ) idx = 0;
		if ( idx >= static_cast<int>( k_gmDrums.size() ) ) idx = static_cast<int>( k_gmDrums.size() ) - 1;
		return k_gmDrums[ idx ].note;
	}
